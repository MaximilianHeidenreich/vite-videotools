...
var require_node_ffmpeg = __commonJS((exports, module) => {
  module.exports = __require("./node-ffmpeg-aa7550s9.node");
});
...
// src/transformer.ts
var import_ffmpeg = __toESM(require_lib(), 1);
...
class VideoTransformer {
  ...
  async transformIntoURL({ directives }) {
    ...
    let exists = false;
    try {
      await fs2.access(outputFile, fsConstants.F_OK);
      exists = true;
    } catch {
      exists = false;
    }
...
function videotools(userOptions = {}) {
  ...
  return {
    ...
    async load(id) {
      ...
      const srcFile = path2.parse(pathname);
